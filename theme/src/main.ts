...
createApp(App).use(router).use(createPinia()).use(i18n).use(VueApexCharts).use(quillEditor).use(rate).use(PerfectScrollbar).use(Lightbox).use(AosVue).use(VueMasonryPlugin).use(VueNumber).use(vueChartist).use(SimpleTypeahead).component(VueFeather.name, VueFeather).component("Breadcrumbs", Breadcrumbs).component("Datepicker", Datepicker).component("multiselect", Multiselect).mount("#app");
