<template>
  <div v-if="steps.length > 0" :key="selectedTarget.id" :data-id="dataId" class="bg-white p-6 rounded-lg shadow-md">
    <!-- Step indicator with labels -->
    <div class="mb-8">
      <div class="flex justify-between mb-2">
        <div v-for="(step, index) in totalSteps" :key="index" class="flex flex-col items-center flex-1">
          <div class="flex items-center w-full">
            <!-- Step circle -->
            <div class="w-8 h-8 rounded-full flex items-center justify-center border-2 font-medium" :class="[index < currentStep ? 'border-sky-500 bg-sky-500 text-white' : index === currentStep ? 'border-sky-500 text-sky-500' : 'border-gray-300 text-gray-300']">
              {{ index + 1 }}
            </div>
            <!-- Connecting line -->
            <div v-if="index < totalSteps - 1" class="flex-1 h-0.5 mx-2" :class="index < currentStep ? 'bg-sky-500' : 'bg-gray-300'"></div>
          </div>
          <!-- Step label -->
          <span class="text-xs mt-2 font-medium" :class="index <= currentStep ? 'text-sky-500' : 'text-gray-400'">
    ...
    <div v-if="currentStepFields" class="mb-6 transition-all duration-300 ease-in-out">
      ...
      <div v-if="isInline && currentStep === 0" class="space-y-4 mb-6">
        <div class="form-group">
          ...
          <select class="w-full p-3 border border-gray-300 rounded-lg bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent" name="location" id="location" v-model="selectedPosition" @change="onSelectPosition">
        ...
        <div class="form-group">
          ...
          <select class="w-full p-3 border border-gray-300 rounded-lg bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent" name="where" id="where" v-model="selectedRelativePosition">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div v-for="(field, key) in currentStepFields" :key="key" class="form-group">
          ...
          <template v-if="field.type === 'color'">
            <input type="color" class="w-full p-1 h-10 border border-gray-300 rounded-lg bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent" :name="key" :id="key" :value="field.value" @input="(e) => handleInputChange(currentStepKey + '.' + key, e.target.value)" />
          </template>

          <template v-else-if="field.type === 'date'">
            <input type="datetime-local" class="w-full p-3 border border-gray-300 rounded-lg bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent" :name="key" :id="key" :value="field.value" @input="(e) => handleInputChange(currentStepKey + '.' + key, e.target.value)" />
          </template>

          <template v-else-if="field.type === 'number'">
            <input type="number" class="w-full p-3 border border-gray-300 rounded-lg bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent" :name="key" :id="key" :value="field.value" @input="(e) => handleInputChange(currentStepKey + '.' + key, e.target.value)" />
          </template>

          <template v-else-if="field.type === 'url'">
            <input type="url" class="w-full p-3 border border-gray-300 rounded-lg bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent" :name="key" :id="key" :value="field.value" placeholder="https://" @input="(e) => handleInputChange(currentStepKey + '.' + key, e.target.value)" />
          </template>

          <template v-else>
            <input type="text" class="w-full p-3 border border-gray-300 rounded-lg bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent" :name="key" :id="key" :value="field.value" @input="(e) => handleInputChange(currentStepKey + '.' + key, e.target.value)" />
          </template>
        </div>
      </div>
    </div>
    ...
    <div class="flex justify-between mt-8 pt-4 border-t border-gray-100">
      <button v-if="currentStep > 0" class="px-6 py-2.5 rounded-lg text-gray-600 hover:text-gray-800 hover:bg-gray-100 transition-colors duration-200 flex items-center gap-2" @click="prevStep">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
          <path fill-rule="evenodd" d="M9.707 16.707a1 1 0 01-1.414 0l-6-6a1 1 0 010-1.414l6-6a1 1 0 011.414 1.414L5.414 9H17a1 1 0 110 2H5.414l4.293 4.293a1 1 0 010 1.414z" clip-rule="evenodd" />
        </svg>
        Geri
      </button>
      <button v-if="currentStep < totalSteps - 1" class="px-6 py-2.5 bg-sky-500 text-white rounded-lg hover:bg-sky-600 transition-colors duration-200 flex items-center gap-2" @click="nextStep">
        İleri
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
          <path fill-rule="evenodd" d="M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z" clip-rule="evenodd" />
        </svg>
      </button>
      <button v-else class="px-6 py-2.5 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors duration-200 flex items-center gap-2" @click="onPreview">
        Preview
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
