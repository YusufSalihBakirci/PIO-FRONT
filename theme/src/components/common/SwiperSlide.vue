<template>
    <Card1 :colClass="colClass" cardClass="card" v-for="(items, index) in data" :key="index">
...
const increment = (index: number, childIndex: number) => {
    props.data[index].children[childIndex].value++;
};

const decrement = (index: number, childIndex: number) => {
    if (props.data[index].children[childIndex].value > 0) {
        props.data[index].children[childIndex].value--;
    }
}
</script>
